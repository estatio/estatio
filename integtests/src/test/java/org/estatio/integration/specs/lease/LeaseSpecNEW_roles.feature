...
Feature: Manage AgreementRoles for an Agreement (REWORKED)

#
# some principles (thrashed out between Jeroen and Dan)
#
# 1. null/null  -> can add after [explicit, null)
# 2. null/null  -> can add before (null,explicit
# 3. start/null -> can add after
# 4. start/null -> can add before, but must be contiguous (..., start]
# 5. ???/end    -> can add after, but must be contiguous [end, ...)
# 6. if two roles, can slot a new one in between, but can only modify one of the existing (eg make one end earlier, or make one start later)
#
