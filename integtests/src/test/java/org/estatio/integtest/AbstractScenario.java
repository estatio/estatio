...
/**
 * Provides a mechanism to share information between {@link AbstractEstatioCukeStepDefs step definitions}.
 * 
 * <p>
 * Basically this is a glorified {@link Map map} of identified objects, bound to the current thread.
 */
public class AbstractScenario<T extends AbstractApp>  {

    
    private static ThreadLocal<AbstractScenario<?>> current = new ThreadLocal<AbstractScenario<?>>();
    
    public static AbstractScenario<?> current() {
        final AbstractScenario<?> scenario = current.get();
        if(scenario == null) {
