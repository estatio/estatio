...
public class BudgetKeyTables extends UdoDomainRepositoryAndFactory<BudgetKeyTable> {
    ...
    @Programmatic
    public BudgetKeyTable findOrCreateBudgetKeyTable(
            final Property property,
            final String name,
            final LocalDate startDate,
            final LocalDate endDate,
        ...
        if (findBudgetKeyTableByName(name)!=null) {
            return findBudgetKeyTableByName(name);
        } else {
            return newBudgetKeyTable(property, name, startDate, endDate, foundationValueType, keyValueMethod, numberOfDigits);
        }
    }
