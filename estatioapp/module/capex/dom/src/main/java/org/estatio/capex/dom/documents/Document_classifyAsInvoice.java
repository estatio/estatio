...
public class Document_classifyAsInvoice extends Document_classifyAsAbstract {
    ...
    @Mixin(method="act")
    public static class Task_categoriseAsInvoice
            extends
            Task._mixinAbstract<Document_classifyAsInvoice, Document> {

        protected final Task task;
        public Task_categoriseAsInvoice(final Task task) {
            super(task, Document_classifyAsInvoice.class);
            this.task = task;
        }

        public static class DomainEvent extends EstatioCapexDomModule.ActionDomainEvent<Task_categoriseAsInvoice> {}
