...
public class LeaseItemForPercentageBuilder extends BuilderScriptAbstract<LeaseItem, LeaseItemForPercentageBuilder> {

    public static final LeaseItemType LEASE_ITEM_TYPE = LeaseItemType.RENTAL_FEE;
    ...
    @Getter @Setter List<LeaseTermForPercentageBuilder.TermSpec> termSpecs = Lists.newArrayList();

    @Getter LeaseItem object;
    @Getter @Setter List<LeaseTermForPercentage> terms = Lists.newArrayList();

    @Override
    protected void execute(final ExecutionContext ec) {

        defaultParam("charge", ec, Charge_enum.ItPercentage.findUsing(serviceRegistry));

        final ChargeGroup group = getCharge().getGroup();
        final ChargeGroup_enum expectedGroup = ChargeGroup_enum.Percentage;
        ...
        for (LeaseTermForPercentageBuilder.TermSpec termSpec : termSpecs) {
            final LeaseTermForPercentage term = new LeaseTermForPercentageBuilder()
                    ...
                    .setPercentage(termSpec.percentage)
                    .build(this, ec)
                    .getObject();
            terms.add(term);
        }
