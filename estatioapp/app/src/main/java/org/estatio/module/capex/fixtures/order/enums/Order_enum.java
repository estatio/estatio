...
public enum Order_enum
    ...
    fakeOrder2Pdf(
            Person_enum.DylanOfficeAdministratorGb, IncomingPdf_enum.FakeOrder2, ApplicationTenancy_enum.Gb,
            new DateTime(2014, 3, 5, 10, 0), "estatio-user-gb", Organisation_enum.TopModelGb,
            Organisation_enum.HelloWorldGb, Project_enum.OxfProject, PropertyAndUnitsAndOwnerAndManager_enum.OxfGb,
            ld(2014, 3, 6),
            Tax_enum.GB_VATSTD,
            "order item", bd("1000.00"), bd("210.00"), bd("1210.00"), IncomingCharge_enum.FrWorks, "F2016",
            "marketing stuff", bd("500.00"), bd("105.00"), bd("605.00"), IncomingCharge_enum.FrMarketing, "F2017"
    );
    ...
    private final String orderItem1Description;
    private final BigDecimal orderItem1NetAmount;
    private final BigDecimal orderItem1VatAmount;
    private final BigDecimal orderItem1GrossAmount;
    private final IncomingCharge_enum item1Charge_d;
    private final String orderItem1Period;

    private final String orderItem2Description;
    private final BigDecimal orderItem2NetAmount;
    private final BigDecimal orderItem2VatAmount;
    private final BigDecimal orderItem2GrossAmount;
    private final IncomingCharge_enum item2Charge_d;
    private final String orderItem2Period;

    @Override
    public OrderBuilder builder() {
        return new OrderBuilder()
                .setPrereq((f,ec) -> f.setOfficerAdministrator(f.objectFor(officerAdministrator_d, ec)))
                .setPrereq((f,ec) -> {
                ...
                .setPrereq((f,ec) -> f.setOrderItemTax(f.objectFor(itemTax_d, ec)))
                .setPrereq((f,ec) -> f.setOrderItem1Description(orderItem1Description))
                .setPrereq((f,ec) -> f.setOrderItem1NetAmount(orderItem1NetAmount))
                .setPrereq((f,ec) -> f.setOrderItem1VatAmount(orderItem1VatAmount))
                .setPrereq((f,ec) -> f.setOrderItem1GrossAmount(orderItem1GrossAmount))
                .setPrereq((f,ec) -> f.setOrderItem1Charge(f.findUsing(item1Charge_d)))
                .setPrereq((f,ec) -> f.setOrderItem1Period(orderItem1Period))

                .setPrereq((f,ec) -> f.setOrderItem2Description(orderItem2Description))
                .setPrereq((f,ec) -> f.setOrderItem2NetAmount(orderItem2NetAmount))
                .setPrereq((f,ec) -> f.setOrderItem2VatAmount(orderItem2VatAmount))
                .setPrereq((f,ec) -> f.setOrderItem2GrossAmount(orderItem2GrossAmount))
                .setPrereq((f,ec) -> f.setOrderItem2Charge(f.findUsing(item2Charge_d)))
                .setPrereq((f,ec) -> f.setOrderItem2Period(orderItem2Period))
                ;
    }
