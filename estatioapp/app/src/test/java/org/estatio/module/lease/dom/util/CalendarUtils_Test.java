 ...
 *  under the License.
 */
package org.estatio.module.lease.dom.util;
...
public class CalendarUtils_Test {
    ...
    public static class IntervalContaining extends CalendarUtils_Test {
        ...
        public void intervalContainingTest() throws Exception {
            Assert.assertEquals(
                    CalendarUtils.intervalContaining(new LocalDate(2012, 2, 1), "RRULE:FREQ=MONTHLY;INTERVAL=3"), new Interval(new LocalDate(2012, 1, 1).toInterval().getStartMillis(), new LocalDate(2012, 4, 1).toInterval().getStartMillis()));
        }
