...
The `T_paperclips` mixin collection returns the list of paperclips for the specified domain object.
...
The service is called when creating a `Document` from a provided template and input domain object, and is defined as:
...
The default implementation of this service is provided.  This simply returns the document name if explicitly provided, otherwise uses the title of the domain object concatenated with the name of the template.  In both cases the appropriate
file suffix is tacked on if required:

[source,java]
----
public class DocumentNamingService.Default {
    public String nameOf(
            final String documentName,
            final Object domainObject,
            final DocumentTemplate template) {
