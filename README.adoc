...
At about the same level of abstraction are a modules provided by the http://catalog.incode.org[Incode Catalog].
...
On top of these are a number of Wicket UI components, also provided by http://isisaddons.org[Isis Addons].
...
This also consists of further maven module in _this_ git repo, notably the `estatio-app` module (defining the `EstatioAppManifest` and a number of menu services) and the `estatio-webapp` module (which configures the various UI components and builds the WAR file).
...
+
These are re-released every time there is a new release of Apache Isis itself.
...
These are split into different to eliminate cyclic dependencies between modules (avoid the big all of mud). +
+
...
(If this isn't the case, then it suggests that the boundaries between the modules themselves are wrong, so should be reworked).
Let's consider two different scenarios:

* a change to functionality within the Estatio domain modules (in this git repo). +
+
In this case, the developer can just make and test the change, there's not much more to be said...

* a change to functionality within an http://isisaddons.org[Isis Addons]' (technical) modules, or an http://catalog.incode.org[Incode Catalog] (business) modules. +
+
...
Estatio's CI server (on CloudBees) will then create a new timestamped build, eg `1.13.0.20161017-1231`; this is published to the Snapshot repository.  
