                ...
                if (item.getInvoicedBy() == LeaseAgreementRoleTypeEnum.LANDLORD && item.getStatus()!= LeaseItemStatus.SUSPENDED && item.getStatus()!= LeaseItemStatus.TERMINATED) {
                    ...
                    switch (type) {
                        ...
                        case MARKETING:
                            LOG.info(
                                    String.format(
                                            "Creating item of type %s for %s with startdate %s and frequency %s while ending item with frequency %s",
                                            type,
                                            lease.getReference(),
                            ...
                            leaseAmendmentService.closeOriginalAndOpenNewLeaseItem(startDateNewItem, item, newInvoicingFrequency);
                        break;
